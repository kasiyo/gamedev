...
void CGame::Initialize() {
    ...
    //m_MainMenuButtons.push_back(Button(Vector2(300, 200), Vector2(200, 50), "Start Game", XMFLOAT4(0.0f, 1.0f, 0.0f, 1.0f)));
    //m_MainMenuButtons.push_back(Button(Vector2(300, 300), Vector2(200, 50), "Exit", XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f)));

    //BeginGame();
    m_eGameState = eGameState::MainMenu; // Set initial state to MainMenu
} //Initialize

void CGame::RenderMainMenu() {
    m_pRenderer->BeginFrame();


    // Draw menu text
    m_pRenderer->DrawScreenText("Main Menu", Vector2(300.0f, 200.0f));
    m_pRenderer->DrawScreenText("Press Enter to Start", Vector2(300.0f, 250.0f));
    m_pRenderer->DrawScreenText("Press L to Load", Vector2(300.0f, 300.0f));
...
void CGame::SaveGame(const std::string& filename) {
    ...
    file << "<GameState>\n";
    m_pTileManager->SaveState(file); // Save the tile manager state
...
void CGame::LoadGame(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        if (line.find("<Tile") != std::string::npos) {
            int x, y;
            char type;
            sscanf(line.c_str(), "<Tile x=\"%d\" y=\"%d\" type=\"%c\"/>", &x, &y, &type);

            // Restore the tile state
            Tile* tile;
            if (m_pTileManager->GetTile(x, y, &tile)) {
                tile->type = type;
            }
        }
    }
...
void CGame::RenderPauseMenu() {
    ...
    m_pRenderer->DrawScreenText("Game Paused", Vector2(300.0f, 200.0f));
    m_pRenderer->DrawScreenText("Press R to Resume", Vector2(300.0f, 250.0f));
    m_pRenderer->DrawScreenText("Press Q to Quit", Vector2(300.0f, 300.0f));
    m_pRenderer->DrawScreenText("Press S to Save and Return to Main Menu", Vector2(300.0f, 350.0f));
...
void CGame::KeyboardHandler() {
    ...
    if (m_eGameState == eGameState::MainMenu) {
        if (m_pKeyboard->TriggerDown(VK_RETURN)) {
            m_eGameState = eGameState::Playing;
            BeginGame(); // Start a new game
        }

        if (m_pKeyboard->TriggerDown('L')) {
            LoadGame("SaveFile.xml"); // Load the saved game
            m_eGameState = eGameState::Playing; // Transition to the game state
        }
    }

    if (m_pKeyboard->TriggerDown('P')) {
        if (m_eGameState == eGameState::Playing) {
            m_eGameState = eGameState::Paused; // Pause the game
        }
        else if (m_eGameState == eGameState::Paused) {
            m_eGameState = eGameState::Playing; // Resume the game
        }
    }

    // Additional controls for the pause menu
    if (m_eGameState == eGameState::Paused) {
        ...
        if (m_pKeyboard->TriggerDown('S')) {
            SaveGame("SaveFile.xml"); // Save the game state
            m_eGameState = eGameState::MainMenu; // Go to the main menu
        }
    }
...
void CGame::ProcessFrame() {
    ...
    if (m_eGameState == eGameState::Paused) {
        RenderPauseMenu();
        KeyboardHandler(); // Handle resume and quit input
        return;
    }
